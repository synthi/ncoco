	...
	*new { arg context, doneCallback;
	...
	alloc {
		...
		SynthDef(\NcocoDSP, {
            ...
            globalChaos=0, 
            coco1OutMode=0, coco2OutMode=0, // 0=Env, 1=Audio
            cocoSlewL=0.1, cocoSlewR=0.1,   // Output Slew Control

			slew_speed=0.1, slew_amp=0.05, slew_misc=0;

			// --- VARS DEFINITION (STRICT ORDER) ---
			var dest_gains;
            ...
            // Independent physics variables for Input (Env) and Output (Coco)
            ...
            // PHYSICS A: INPUT ENVELOPE (Linked-Ratio)
            // Driven by envSlewL/R
            relInL = envSlewL.linexp(0, 1, 0.05, 2.5);
            relInR = envSlewR.linexp(0, 1, 0.05, 2.5);
            atkInL = (relInL * 0.1).max(0.002);
            atkInR = (relInR * 0.1).max(0.002);
            ...
            // 3. MATRIX DEFINITION (AUDIO RATE)
            sources_sig = [
                fb_petals[0], fb_petals[1], fb_petals[2], 
                fb_petals[3], fb_petals[4], fb_petals[5], 
                K2A.ar(envL), K2A.ar(envR), 
                fb_yellow[0], fb_yellow[1], 
                fb_src11, fb_src12 
            ];

            // 4. PETAL MODULATION
			mod_p1_Amts=NamedControl.kr(\mod_p1_Amts, 0!12); mod_p2_Amts=NamedControl.kr(\mod_p2_Amts, 0!12);
			mod_p3_Amts=NamedControl.kr(\mod_p3_Amts, 0!12); mod_p4_Amts=NamedControl.kr(\mod_p4_Amts, 0!12);
			mod_p5_Amts=NamedControl.kr(\mod_p5_Amts, 0!12); mod_p6_Amts=NamedControl.kr(\mod_p6_Amts, 0!12);

            // CHAOS OFFSET LOGIC
            p1c = (p1chaos + globalChaos).clip(0, 1);
            p2c = (p2chaos + globalChaos).clip(0, 1);
            p3c = (p3chaos + globalChaos).clip(0, 1);
            p4c = (p4chaos + globalChaos).clip(0, 1);
            p5c = (p5chaos + globalChaos).clip(0, 1);
            p6c = (p6chaos + globalChaos).clip(0, 1);
			...
			mod_speedL_Amts=NamedControl.kr(\mod_speedL_Amts, 0!12); mod_speedR_Amts=NamedControl.kr(\mod_speedR_Amts, 0!12);
			...
			mod_flipL_Amts=NamedControl.kr(\mod_flipL_Amts, 0!12); mod_flipR_Amts=NamedControl.kr(\mod_flipR_Amts, 0!12);
            ...
            mod_skipL_Amts=NamedControl.kr(\mod_skipL_Amts, 0!12); mod_skipR_Amts=NamedControl.kr(\mod_skipR_Amts, 0!12);
            ...
            // NOISE SPLIT: 50% at Output
			readL = readL + bleedL + (noiseL * 0.5); 
            readR = readR + bleedR + (noiseR * 0.5);

            // --- SOURCES 11 & 12 CALCULATION (CORRECTED v10002) ---
            
            // PHYSICS B: COCO ENVELOPE (Linked-Ratio)
            // Driven by cocoSlewL/R (Corrected)
            relCoL = cocoSlewL.linexp(0, 1, 0.05, 2.5);
            relCoR = cocoSlewR.linexp(0, 1, 0.05, 2.5);
            atkCoL = (relCoL * 0.1).max(0.002);
            atkCoR = (relCoR * 0.1).max(0.002);
            
            // Use LeakDC to prevent offset issues on feedback read
            ...
            // --- REST OF MODULATIONS (AR) ---
            // All mod arrays now size 12
			mod_ampL_Amts=NamedControl.kr(\mod_ampL_Amts, 0!12); mod_ampR_Amts=NamedControl.kr(\mod_ampR_Amts, 0!12);
			...
			mod_fbL_Amts=NamedControl.kr(\mod_fbL_Amts, 0!12); mod_fbR_Amts=NamedControl.kr(\mod_fbR_Amts, 0!12);
			...
			mod_filtL_Amts=NamedControl.kr(\mod_filtL_Amts, 0!12); mod_filtR_Amts=NamedControl.kr(\mod_filtR_Amts, 0!12);
			...
			mod_volL_Amts=NamedControl.kr(\mod_volL_Amts, 0!12); mod_volR_Amts=NamedControl.kr(\mod_volR_Amts, 0!12);
			...
			mod_audioInL_Amts=NamedControl.kr(\mod_audioInL_Amts, 0!12); mod_audioInR_Amts=NamedControl.kr(\mod_audioInR_Amts, 0!12);
            ...
            mod_recL_Amts=NamedControl.kr(\mod_recL_Amts, 0!12); mod_recR_Amts=NamedControl.kr(\mod_recR_Amts, 0!12);
        ...
        // COMMANDS UPDATED
        this.addCommand("coco1_out_mode", "i", { |msg| synth.set(\coco1OutMode, msg[1]) });
        this.addCommand("coco2_out_mode", "i", { |msg| synth.set(\coco2OutMode, msg[1]) });
        this.addCommand("global_chaos", "f", { |msg| synth.set(\globalChaos, msg[1]) });
        this.addCommand("cocoSlewL", "f", { |msg| synth.set(\cocoSlewL, msg[1]) });
        this.addCommand("cocoSlewR", "f", { |msg| synth.set(\cocoSlewR, msg[1]) });
        ...
        // MODULATION COMMANDS UPDATED TO 12 ARGS
		this.addCommand("mod_speedL", "ffffffffffff", { |msg| synth.setn(\mod_speedL_Amts, msg.drop(1)) });
		this.addCommand("mod_speedR", "ffffffffffff", { |msg| synth.setn(\mod_speedR_Amts, msg.drop(1)) });
		this.addCommand("mod_flipL", "ffffffffffff", { |msg| synth.setn(\mod_flipL_Amts, msg.drop(1)) });
		this.addCommand("mod_flipR", "ffffffffffff", { |msg| synth.setn(\mod_flipR_Amts, msg.drop(1)) });
		this.addCommand("mod_skipL", "ffffffffffff", { |msg| synth.setn(\mod_skipL_Amts, msg.drop(1)) });
		this.addCommand("mod_skipR", "ffffffffffff", { |msg| synth.setn(\mod_skipR_Amts, msg.drop(1)) });
		this.addCommand("mod_ampL", "ffffffffffff", { |msg| synth.setn(\mod_ampL_Amts, msg.drop(1)) });
		this.addCommand("mod_ampR", "ffffffffffff", { |msg| synth.setn(\mod_ampR_Amts, msg.drop(1)) });
		this.addCommand("mod_fbL", "ffffffffffff", { |msg| synth.setn(\mod_fbL_Amts, msg.drop(1)) });
		this.addCommand("mod_fbR", "ffffffffffff", { |msg| synth.setn(\mod_fbR_Amts, msg.drop(1)) });
		this.addCommand("mod_filtL", "ffffffffffff", { |msg| synth.setn(\mod_filtL_Amts, msg.drop(1)) });
		this.addCommand("mod_filtR", "ffffffffffff", { |msg| synth.setn(\mod_filtR_Amts, msg.drop(1)) });
		this.addCommand("mod_recL", "ffffffffffff", { |msg| synth.setn(\mod_recL_Amts, msg.drop(1)) });
		this.addCommand("mod_recR", "ffffffffffff", { |msg| synth.setn(\mod_recR_Amts, msg.drop(1)) });
		this.addCommand("mod_volL", "ffffffffffff", { |msg| synth.setn(\mod_volL_Amts, msg.drop(1)) });
		this.addCommand("mod_volR", "ffffffffffff", { |msg| synth.setn(\mod_volR_Amts, msg.drop(1)) });
		this.addCommand("mod_p1", "ffffffffffff", { |msg| synth.setn(\mod_p1_Amts, msg.drop(1)) });
		this.addCommand("mod_p2", "ffffffffffff", { |msg| synth.setn(\mod_p2_Amts, msg.drop(1)) });
		this.addCommand("mod_p3", "ffffffffffff", { |msg| synth.setn(\mod_p3_Amts, msg.drop(1)) });
		this.addCommand("mod_p4", "ffffffffffff", { |msg| synth.setn(\mod_p4_Amts, msg.drop(1)) });
		this.addCommand("mod_p5", "ffffffffffff", { |msg| synth.setn(\mod_p5_Amts, msg.drop(1)) });
		this.addCommand("mod_p6", "ffffffffffff", { |msg| synth.setn(\mod_p6_Amts, msg.drop(1)) });
		this.addCommand("mod_audioInL", "ffffffffffff", { |msg| synth.setn(\mod_audioInL_Amts, msg.drop(1)) });
		this.addCommand("mod_audioInR", "ffffffffffff", { |msg| synth.setn(\mod_audioInR_Amts, msg.drop(1)) });

...
function GridNav.key(G, g, x, y, z)
  ...
  if z==1 and not G.focus.source then
    ...
    elseif obj.t=='fader' then 
        local spd = G.SPEED_TABLE[obj.m]
        params:set("speed"..side, spd) 
    end
  elseif z==0 then 
    if obj.t=='skip' then 
      local side = (obj.id==1) and "L" or "R"
    ...
    if obj then
      ...
      elseif obj.t=='jack' or obj.t=='p_jack' then
        ...
        elseif G.focus.inspect_dest==obj.id then b=15
          ...
          if G.patch[1][obj.id]~=0 then b=math.max(b, 8) end 
        end 
      ...
      elseif obj.t=='flip' then 
        local c=G.coco[obj.id]
        local is_rev = (c.gate_flip and c.gate_flip < 0)
        if GridNav.cache[x][y] == 15 then b=15
        else b = is_rev and 10 or 4 end
        
      elseif obj.t=='skip' then 
        local c=G.coco[obj.id]
        if GridNav.cache[x][y] == 15 then b=15 else b=4 end

      elseif obj.t=='fader' then local c=G.coco[obj.id]; b = get_fader_bright(G, c.real_speed or 1.0, obj.m)
      else b=4 end
    end
